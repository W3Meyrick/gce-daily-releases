...
__log = get_logger(__name__, '/var/log/ocean-ip-enforcer.log', False)
...
def project_ids_list(folderId):
    projects_to_exclude = exclusions_from_bucket()
    client = resource_manager.Client()
    project_filter = { 'parent.type': 'folder', 'parent.id': folderId }
    projects = list(client.list_projects(project_filter))
    project_ids = []
        ...
        for item in v['addresses']:
            if item['addressType'] == "EXTERNAL":
                ...
                if status == "RESERVED" and not regional:
                    ...
                    try:
                        # delete_global_address_reservation(service, project, name)
                    ...
                    if not regional:
                        try:
                            ...
                            __log.warning("OCEAN deleted IP address {} and attached resource {} from {}".format(address, consumer_name, project))
                        except Exception as e:
                            __log.error(e)
                    else:
                        try:
                            ...
                            # delete_address_reservation(service, project, region, item['name'])
                            __log.warning("OCEAN deleted IP address {} and attached resource {} from {}".format(address, consumer_name, project))
                        except Exception as e:
                            __log.error(e)
                ...
                # TODO: Add functionality for instance groups.    
                    ...
                    __log.error("OCEAN: The IP address {} is in use on {} which is not supported.".format(address, zone))
...
def main():
    ...
    if __FUNCTION_PROJECT_ID == 'hsbc-6320774-enforcer-test':
        ...
        folderId = "464555476602"
        projects = project_ids_list(folderId)
    elif __FUNCTION_PROJECT_ID == 'hsbc-6320774-enforcer-prod':
        folderId = "962521782257"
        projects = project_ids_list(folderId)
    else:
        __log.error(__FUNCTION_PROJECT_ID + ' is not a valid OCEAN project')
        raise InvalidGcpProject(__FUNCTION_PROJECT_ID + ' is not a valid OCEAN project')
